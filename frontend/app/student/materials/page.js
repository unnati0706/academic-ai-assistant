  ...
  return (
    <div className="space-y-5 sm:space-y-6 max-w-7xl mx-auto text-[#f5efeb]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-[#f5efeb] tracking-tight">Academic Resource Explorer</h1>
        </div>
      ...
      {loading ? (
        <div className="py-20 flex justify-center items-center space-x-3">
          ...
          <span className="text-xs text-[#c8bfb8] font-medium">Fetching study materials...</span>
        </div>
      ) : materials.length === 0 ? (
        <div className="glass-panel rounded-2xl p-10 sm:p-12 text-center border border-[#463c39] bg-[#312a28]/60">
          <BookOpen className="w-12 h-12 text-[#b5c7d3]/40 mx-auto mb-3" />
          <h3 className="text-sm font-bold text-[#f5efeb]">No uploaded materials found</h3>
            ...
            >
              <div>
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  {mat.unit && (
                    <span className="text-[11px] font-semibold text-[#c8bfb8] bg-[#282220] px-2 py-0.5 rounded-md border border-[#463c39]">
                      Unit {mat.unit}
                    </span>
                  )}
                </div>
                ...
                <p className="text-[11px] text-[#b5c7d3] font-medium mt-1">{mat.subject_name || mat.subject_code || 'Computer Science'}</p>

                {mat.uploader && (
                  <div className="flex items-center space-x-1.5 text-[11px] text-[#c8bfb8] mt-2">
                    <User className="w-3 h-3 text-[#b5c7d3]" />
                    <span>Uploaded by: {mat.uploader.email || 'Faculty'}</span>
                  </div>
                )}

                <p className="text-xs text-[#c8bfb8] mt-2 line-clamp-3 leading-relaxed">
